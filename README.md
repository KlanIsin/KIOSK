...
3. Add school documents to the `knowledge_base/` folder.
   - You may now place text-based PDF files in `knowledge_base/` as well as `.txt` files.
...
## Add school context
Create `.txt` files inside `knowledge_base/` with policies, schedules, contact info, student services, and other school-specific details.
